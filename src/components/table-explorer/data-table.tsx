...
function formatCellValue(value: any): { value: string; isNull: boolean } {
...
export function DataTable({
  ...
  const handleLinkClick = useCallback(
    async (schema: string, table: string, column: string, value: string) => {
      try {
        setIsLoading(true);
        const result = await fetchLinkedRecord(schema, table, column, value);
  ...
  const handleSelectAll = useCallback(
    (checked: boolean) => {
      if (checked) {
        // Assuming each row has an id field - adjust according to your primary key
